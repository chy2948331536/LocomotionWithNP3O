...
void lowState::dataToWirelessRemoteState(const std::vector<uint8_t> &data) {
  ...
  wirelessdata.ly = hex_to_float_2B(data, 24, 25);
}

void lowState::dataToImu(const std::vector<uint8_t> &data) {
  for (int i = 0; i < 4; i++) {
    // std::cout << "i * 4: " << i * 4 << " (i + 1) * 4: " << (i + 1) * 4 << std::endl;
    imu_quaternion[i] = hex_to_float_i(data, i * 4, (i + 1) * 4);
    // std::cout << "imu_quaternion[" << i << "]: " << imu_quaternion[i] << std::endl;
  }
...
void lowState::dataToMotorState(const std::vector<uint8_t> &data, int index) {
  ...
  tauEst = static_cast<float>(hex_to_u16_i(data, 11, 13)) * 0.00390625;
...
void lowState::parseData(const std::vector<uint8_t> &data) {
  try {
    ...
    // std::cout << "dataToBmsState(bms_data);" << std::endl;
    ...
    // std::cout << "dataToWirelessRemoteState(wirelessRemote);" << std::endl;
